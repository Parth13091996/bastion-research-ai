// Lightweight Google Sheets utilities for public sheets
// - Parses a Google Sheet URL to extract spreadsheet ID and sheet gid
// - Fetches data via the public GViz endpoint and converts to array of objects

export type RowObject = Record<string, any>;

export const parseSheetUrl = (urlString: string): { id: string; gid: string } => {
  ...
  if (!id) throw new Error('Invalid Google Sheets URL: spreadsheet ID not found');
  return { id, gid };
};
...
export const fetchSheetObjects = async (sheetUrl: string): Promise<RowObject[]> => {
  try {
    ...
    const headers: string[] = (table.cols || []).map((c: any) => (c?.label ?? '').toString());
...
export const toNumber = (v: any): number => {
  ...
  if (v == null) return 0;
  const s = String(v).replace(/[,₹%\s]/g, '').replace(/rs\.?/gi, '').replace(/cr\.?/gi, '');
