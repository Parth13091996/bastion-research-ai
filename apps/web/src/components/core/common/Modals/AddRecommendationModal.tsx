...
function SymbolTagsFields({
  ...
  errors: any;
  localError: Record<string, string>;
  watch: any;
  setValue: any;
  sheetStocks: { id: string; code: string }[];
}) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div>
        ...
        <Input value={watch("video")} onChange={e => setValue("video", e.target.value)} placeholder="https://youtube.com/..." />
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="flex gap-3">
        <Input
          ...
          readOnly
          placeholder={required ? "No file selected" : "Optional"}
...
function PerformanceUrlsTable({
  items,
  setItems,
  error,
}: {
  items: StockPerformanceItem[];
  setItems: React.Dispatch<React.SetStateAction<StockPerformanceItem[]>>;
  error?: string;
}) {
  return (
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <p className="text-center text-gray-500 py-4">No performance URLs added yet.</p>
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
...
function UpdatesTable({
  ...
  title: string;
  items: UpdateItem[];
  setItems: React.Dispatch<React.SetStateAction<UpdateItem[]>>;
  uploading: Record<number, boolean>;
  pdfInputRefs: React.MutableRefObject<(HTMLInputElement | null)[]>;
  onPdfUpload: (index: number, file: File) => void;
  addLabel?: string;
}) {
  return (
    <div>
                ...
                <TableCell>
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      ...
                      {uploading[index] ? "Uploading..." : "Upload PDF"}
                    </Button>
                    <input
                      ...
                      className="hidden"
                      onChange={e =>
                        e.target.files?.[0] && onPdfUpload(index, e.target.files[0])
                      }
                    />
                    {item.pdf_url && (
                      <a
...
function IterationDetails({
  stockPerformanceItems,
  selectedIterationIndex,
  setSelectedIterationIndex,
  iterationResourceUploading,
  iterationBusinessNoteInputRef,
  iterationQuickBiteInputRef,
  iterationExitRationaleInputRef,
  ...
  iterationQuarterlyPdfInputRefs,
  handleIterationQuarterlyPdfUpload,
}: {
  stockPerformanceItems: StockPerformanceItem[];
  selectedIterationIndex: number;
  setSelectedIterationIndex: React.Dispatch<React.SetStateAction<number>>;
  iterationResourceUploading: Record<string, boolean>;
  iterationBusinessNoteInputRef: React.RefObject<HTMLInputElement>;
  iterationQuickBiteInputRef: React.RefObject<HTMLInputElement>;
  iterationExitRationaleInputRef: React.RefObject<HTMLInputElement>;
  handleIterationResourceUpload: (
    field: "business_note" | "quick_bite" | "exit_rationale",
    file: File
  ) => void;
  updateIterationField: (field: keyof StockPerformanceItem, value: string) => void;
  getIterationQuarterlyUpdates: (index: number) => UpdateItem[];
  setIterationQuarterlyUpdates: (iterationIndex: number, nextUpdates: UpdateItem[]) => void;
  iterationQuarterlyUploading: Record<string, boolean>;
  iterationQuarterlyPdfInputRefs: React.MutableRefObject<(HTMLInputElement | null)[]>;
  handleIterationQuarterlyPdfUpload: (
            ...
            <div>
              ...
              <div className="flex gap-3">
                ...
                <input
                  ...
                  onChange={e => {
                    const file = e.target.files?.[0];
                    if (file) handleIterationResourceUpload("exit_rationale", file);
                  }}
                />
              </div>
            </div>
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-2">
                            ...
                            <input
                              ...
                              onChange={e => {
                                const file = e.target.files?.[0];
                                if (file) handleIterationQuarterlyPdfUpload(selectedIterationIndex, index, file);
                              }}
                            />
                            {update.pdf_url && (
                              <a
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            onClick={() => {
                              const next =
                                getIterationQuarterlyUpdates(selectedIterationIndex).filter(
                                  (_, i) => i !== index
                                );
                              setIterationQuarterlyUpdates(selectedIterationIndex, next);
                            }}
...
const AddRecommendationModal: React.FC<AddRecommendationModalProps> = ({
  ...
  const [iterationResourceUploading, setIterationResourceUploading] = useState<Record<string, boolean>>({});
  ...
  // Reset refs array on every render (needed for table row inputs)
  ...
  // --- Handlers for file uploads and selection logic ---
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 w-[95vw] max-w-7xl max-h-[90vh] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-xl overflow-y-auto z-50">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-8" noValidate>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <DocumentUploader
                  ...
                  error={localError.logo}
                  required
                />
                <DocumentUploader
                  label="Business Note (PDF)"
                  field="business_note"
                  accept="application/pdf"
                  selectedFile={selectedFiles.business_note}
                  inputRef={businessNoteInputRef}
                  onUpload={handleFileSelect}
                  error={localError.business_note}
                  required
                />
                <DocumentUploader
                  label="Quick Bite (PDF)"
                  field="quick_bite"
                  accept="application/pdf"
                  selectedFile={selectedFiles.quick_bite}
                  inputRef={quickBiteInputRef}
                  onUpload={handleFileSelect}
                />
                <DocumentUploader
                  label="Exit Rationale (PDF)"
                  field="exit_rationale"
                  accept="application/pdf"
                  selectedFile={selectedFiles.exit_rationale}
                  inputRef={exitRationaleInputRef}
                  onUpload={handleFileSelect}
                />
              </div>
            </div>
            ...
            <PerformanceUrlsTable
              items={stockPerformanceItems}
              setItems={setStockPerformanceItems}
              error={localError.stock_performance_url}
            />

            {/* Iteration-specific overrides */}
            {stockPerformanceItems.length > 1 && (
              <IterationDetails
                stockPerformanceItems={stockPerformanceItems}
                selectedIterationIndex={selectedIterationIndex}
                setSelectedIterationIndex={setSelectedIterationIndex}
                iterationResourceUploading={iterationResourceUploading}
                iterationBusinessNoteInputRef={iterationBusinessNoteInputRef}
                iterationQuickBiteInputRef={iterationQuickBiteInputRef}
                iterationExitRationaleInputRef={iterationExitRationaleInputRef}
                handleIterationResourceUpload={handleIterationResourceUpload}
                updateIterationField={updateIterationField}
                getIterationQuarterlyUpdates={getIterationQuarterlyUpdates}
                setIterationQuarterlyUpdates={setIterationQuarterlyUpdates}
                iterationQuarterlyUploading={iterationQuarterlyUploading}
                iterationQuarterlyPdfInputRefs={iterationQuarterlyPdfInputRefs}
                handleIterationQuarterlyPdfUpload={handleIterationQuarterlyPdfUpload}
              />
            )}
            ...
            <UpdatesTable
              ...
              items={quarterlyUpdates}
              setItems={setQuarterlyUpdates}
              uploading={quarterlyUploading}
              pdfInputRefs={quarterlyPdfInputRefs}
              onPdfUpload={handleQuarterlyPdfUpload}
              addLabel="Add Update"
            />
            ...
            <UpdatesTable
              ...
              uploading={announcementUploading}
              pdfInputRefs={announcementPdfInputRefs}
