...
export const getRedFlagCompanyStats = async (req: Request, res: Response) => {
  try {
    ...
    const [{ data: company, error: companyError }, { data: submissions, error: submissionsError }] =
      await Promise.all([
        supabase
          .from(COMPANIES_TABLE)
          .select("id,name,created_at")
...
export const getAllRedFlagCompanyStats = async (
  ...
  try {
    const [{ data: companies, error: companiesError }, { data: submissions, error: submissionsError }] =
      await Promise.all([
        supabase
          .from(COMPANIES_TABLE)
          .select("id,name,created_at")
